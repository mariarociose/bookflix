...
class Ranking_libros extends CommonDisplay{
    ...
    componentDidMount(){
            ...
            fetch("http://localhost:4000/libros_ranking",{
                method:"GET",
                headers:{
    ...
    renderContent = () => {
            ...
            if(this.state.libros != undefined){
                titulos = this.state.libros.map((libro) => (
                        ...
                        <tr>
                        <td> {libro.cant_lecturas} </td>
                        <td data-title= 'ISBN' key={libro.id_libro}>{libro.isbn} </td>
                        <td >{libro.titulo}  </td>
