...
import LibroFav from "./librofav";
import Cookie from "js-cookie";
class LibroFavsContainer extends CommonDisplay{
    ...
    componentDidMount(){
      ...
      id_perfil = JSON.parse(id_perfil);
        fetch(`http://localhost:4000/librosFavoritosUser?id_perfil=${id_perfil}`)
        .then((res) => res.json())
        .then((news) => {
          if(news.length == 0) news.mensaje = "No tienes ningun favorito por el momento";
...
export default LibroFavsContainer;
